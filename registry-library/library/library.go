...
func GetRegistryIndex(registryURL string, insecure bool, devfileTypes ...indexSchema.DevfileType) ([]indexSchema.Schema, error) {
...
func GetMultipleRegistryIndices(registryURLs []string, insecure bool, devfileTypes ...indexSchema.DevfileType) []Registry {
	...
	for index, registryURL := range registryURLs {
		go func(chan []indexSchema.Schema, chan error) {
			registryContents, err := GetRegistryIndex(registryURL, insecure, devfileTypes...)
...
func PrintRegistry(registryURLs string, devfileType string, insecure bool) error {
	...
	if devfileType == string(indexSchema.StackDevfileType) {
		registryList = GetMultipleRegistryIndices(registryURLArray, insecure, indexSchema.StackDevfileType)
	} else if devfileType == string(indexSchema.SampleDevfileType) {
		registryList = GetMultipleRegistryIndices(registryURLArray, insecure, indexSchema.SampleDevfileType)
	} else if devfileType == "all" {
		registryList = GetMultipleRegistryIndices(registryURLArray, insecure, indexSchema.StackDevfileType, indexSchema.SampleDevfileType)
	}
...
func PullStackByMediaTypesFromRegistry(registry string, stack string, allowedMediaTypes []string, destDir string, insecure bool) error {
	// Get the registry index
	registryIndex, err := GetRegistryIndex(registry, insecure, indexSchema.StackDevfileType)
	if err != nil {
		return err
	}
...
func PullStackFromRegistry(registry string, stack string, destDir string, insecure bool) error {
	return PullStackByMediaTypesFromRegistry(registry, stack, DevfileAllMediaTypesList, destDir, insecure)
}
